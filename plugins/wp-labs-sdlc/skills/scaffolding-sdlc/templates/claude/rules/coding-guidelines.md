---
alwaysApply: true
description: general coding guidelines for AI code
---
...
As Go Proverbs says: "A little copying is better than a little dependency."
...
### Performance
SQL in loops: be very careful about SQL within a loop (DB calls are expensive).
...
### Always Add Tests
For every feature added or bug fixed, add unit tests covering the core logic and functional/integration tests covering the end-to-end behavior. **For JavaScript/React, tests must be co-located with source files** (e.g. `Button.test.jsx` next to `Button.jsx`). For Python, follow the project's existing test structure.
