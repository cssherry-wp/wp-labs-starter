...
| **sdlc** | The `scaffolding-sdlc` skill: interactively bootstraps a repo's SDLC — a runnable starter app (TypeScript / Python+Django / fullstack Python+React), a Makefile task runner, pre-commit + commit-doc hooks, GitHub Actions for lint/typecheck/unit/Playwright-e2e/security gates, Dependabot, PR-status labels, PR auto-rebase, the Claude review/triage workflows, and an optional hosting layer (Docker + docker-compose + Azure Bicep deploy). On an existing repo it audits what's present and adds only the missing pieces. |
